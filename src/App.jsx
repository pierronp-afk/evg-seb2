...
export default function App() {
  ...
  const handleLogin = async (rawName, personalPin) => {
      ...
      // Mettre à jour l'UID Firebase (utile si connexion depuis un nouvel appareil)
      if (user?.uid) {
        const detailsRef = doc(db, 'artifacts', appId, 'public', 'data', 'participants', 'details');
             ...
             <Card>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                 {participants.map((p, idx) => {
                   ...
                   return (
                   <div key={idx} className={`flex flex-col gap-2 bg-white p-3 rounded-lg border shadow-sm ${isAdminMode ? 'border-green-200' : 'border-gray-100'}`}>
